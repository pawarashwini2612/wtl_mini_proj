...
<body>    
    ...
    <form action="AdminViewCase.jsp" >
        <div class="container">   
            <label>Aarogya Vibhag Name : </label>   
            <input type="text" placeholder="Enter Aarogya Vibhag name" name="name" required>  
            

           
            <button type="submit">Search Aarogya Vibhag </button>   
    ...
    <%
    	...
    	if(name!=null)
    	{
    		try
    		{
    			...
    			while(rs.next())
    			{
    %>				<h3>Aarogya Vibhag ID : <%=rs.getString(1)%></h3>
    				<h3>Email : <%=rs.getString(2)%></h3>
    				<h3>Password : <%=rs.getString(3)%></h3>
    				<h3>Name Of Aarogya Vibhag : <%=rs.getString(4)%></h3>
    				<h3>Mobile Number : <%=rs.getString(5)%></h3>
    				<h3>Village : <%=rs.getString(6)%></h3>
    				<h3>District : <%=rs.getString(7)%></h3>
    				<a href="AdminDeleteCase.jsp?name=<%=rs.getString(1)%>">Delete</a>
    <% 
    			}
    		}
    		catch(Exception e)
